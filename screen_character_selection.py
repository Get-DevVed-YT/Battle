...
class Screen_CharacterSelection (tkinter.Frame):
    ...
    def create_widgets (self):
        ...
        tkinter.Label(self, 
            ...
            font=("Helvetica", 20),
            width=10
            ).grid(row=0, column=2, columnspan=3, sticky=tkinter.N)

        tkinter.Label(self, 
            text="Dexterity",
            font=("Helvetica", 20),
            width=10
            ).grid(row=0, column=3, columnspan=3, sticky=tkinter.N)

        tkinter.Label(self, 
            text="Strength",
            font=("Helvetica", 20),
            width=10
            ).grid(row=0, column=4, columnspan=3, sticky=tkinter.N)
        ...
        for char in self.roster.character_list:
            imageElf = tkinter.PhotoImage(
                file="images/" + char.small_image)
            ...
            tkinter.Label(self,
                        text=str(char.hit_points),
                        font=("Helvetica", 16)
                        ).grid(row=row_num, column=2)
            tkinter.Label(self,
                        text=str(char.dexterity),
                        font=("Helvetica", 16)
                        ).grid(row=row_num, column=3)
            tkinter.Label(self,
                        text=str(char.strength),
                        font=("Helvetica", 16)
                        ).grid(row=row_num, column=4)
        ...
        tkinter.Button(self,
                ...
                command=self.selected_clicked,
                fg="Red",
                bg="Black",
                font=("Helvetica", 16)
                ).grid(row=row_num, column=2, columnspan=3, sticky=tkinter.E)
